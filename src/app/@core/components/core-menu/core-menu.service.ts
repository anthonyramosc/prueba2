...
export class CoreMenuService {
   ...
   */
  constructor(private _router: Router, private _authenticationService: AuthenticationService, private http: HttpClient) {
  ...
  /**
   * 
   * @param key Clave de diccionario
   * @returns 
   */
  // setStateHiddenCurrentMenu(key,menu,submenu): void {
  setStateHiddenCurrentMenu(key, menu, submenu): void {
    

    //Si es -1 no oculta titulo
    if (menu == null) {
      // console.log('Menu key:', this._registry.main[key])
      //Menu tittle SEPS
      this._registry[key] = this._registry.main[key].hidden = true;
    }

    //Submenu
    this._registry.main[key].children[submenu].hidden = true;

  }

  getEstructurasEmpresa(){
    return this.http.get(`${environment.apiUrl}/estructuras`);
  }
